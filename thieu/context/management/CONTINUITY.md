...
_Last refreshed: 2026-07-08 (Booking live + polished: fixed the "Confirm does nothing" dead-click, cleaned up the invite email subject to "Private Consultation — Maison Denude (Online|In Person)", and switched the prod invite organizer to thieuxmaison@gmail.com. Latest commit `f75a602` (+ pending docs commit), deploy `82422b17`, all live-verified on prod. Compaction-safe.)_
...
**Invite email subject + organizer fix (commit `f75a602`, deploy `82422b17`):** the Google Calendar invite title is now `Private Consultation — Maison Denude (Online|In Person)` (English, no client-name cram — set via `summary` in `calendar.ts`; client name + VN/EN detail stay in the event body). The "Invitation from an unknown sender:" prefix + trailing "@ time (organizer)" are Gmail-added and NOT controllable by us. Also: prod Worker Google secrets were STALE (still nguyenthaithieu) so invites organized as the old test account — synced all 5 (GOOGLE_REFRESH_TOKEN/CLIENT_ID/SECRET/CALENDAR_ID + GCAL_OWNER_EMAIL) to thieuxmaison; **verified on prod: organizer = thieuxmaison@gmail.com (self), new subject rendered.** See D-BOOK-EMAIL-01 in booking-calendar.md. **Gotcha: a booking in the first ~10s after `wrangler deploy` can still hit the old Worker bundle — wait for propagation before verifying server output.**
