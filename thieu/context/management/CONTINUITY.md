...
**STILL REMAINING (unchanged, deferred):** (1) decide prod rollout — set GOOGLE_*/GCAL_* as real Worker secrets via `wrangler secret put` on the thieuxmaison CF account + GCAL_SEND_UPDATES=all for prod; (2) promote spec DRAFT → **D-BOOK-04** + OAuth re-mint runbook + residual double-book risk; (3) commit + deploy (NOTHING committed/deployed yet — live site still shows OLD form; migration already on prod DB). ⚠ SECURITY: rotate the pasted OAuth client secret + re-mint post-launch.
