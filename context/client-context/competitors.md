# Maison Denude — Competitors

_Skeleton. Last updated: 2026-06-21. To be populated from Maddy's research + SimilarWeb runs._

## Competitor table

| Brand | Website | Value proposition | USP | Notes |
|-------|---------|-------------------|-----|-------|
| _(TBD)_ | | | | |

## Competitive position notes

_To be populated._

## Source

- Competitor list + SimilarWeb traffic/channel/keyword cues are part of the Week 2 research (Thiệu + Maddy). When compiled, summarize here and keep raw data in `/thieu/context` or `markets/`.
