...
_Append-only: add the next `MD-xxx`; never rewrite history. Last updated: 2026-07-04._
...
| MD-040 | 2026-07-04 | **Website goes live now** — kept in "demo" but public and iterated in place, not held back for perfection | SEO compounds with domain age (Matthew: *"mình càng lên mạng sớm thì mình càng tốt"*); every past press/social mention can now link back to a live site. Chi asked, Matthew advised, **Chi agreed** |
| MD-041 | 2026-07-04 | Current priority = **website + content**; two workstreams run **in parallel** — (A) site structure/design/content, (B) SEO/keyword/content/backlinks. **Paid ads deferred** until the site is complete | Chi chose website+content over ads-distribution-first; Michelle *"làm hết nhưng nên chia giai đoạn"*; no site yet for ads to convert to, and paid social is currently blocked (MD-044) |
| MD-042 | 2026-07-04 | Website structure built **jointly** (Maison + Matthew + Maddy), not handed off. Direction = **two lanes (Ready-to-Wear vs Bespoke/Couture)** + trust pages (press / celebrity / "as seen in") + SEO-structured product pages (e.g. a dedicated "áo dài cưới" page) | Chi wants input on the customer journey/nav; modeled on Qipology's RTW/couture split; peer-house traffic lands on collection/wedding/segment pages, not just home |
| MD-043 | 2026-07-04 | **Adopt Pinterest** as a discovery channel (Chi opens the account; Matthew backlinks it to the site); **keep investing in Facebook/Instagram** for branding | Pinterest is used by ~3/4 of global designer houses and can link images → product pages (IG can't); one peer draws ~36% of referrals from Pinterest. FB/IG stay as branding/reach |
| MD-044 | 2026-07-04 | **Paid social is currently blocked** — Maison's Facebook + Instagram Business Manager is restricted/compromised; ad testing deferred until (a) the site is done and (b) the restriction is resolved. Maddy to seek a fix via agency/ad-network contacts by **~Thu**; fallback = create a new Instagram | Matthew was candid he has no fix yet (same issue on his own hacked account); Chi expects the team's network to help. Adjacency data (luxury-watch/jewelry/hotel buyers) **cannot be bought** — only tested via Google/Meta audience config (Meta > Google), measured by branded-search lift |
| MD-045 | 2026-07-04 | **SEO backlink method:** declare the site's link to authority press domains via structured-data (Matthew named tuoitre.vn, [báo] văn hóa.vn, VTV.vn, vietnam.vn). **Standing process:** team sends every new press mention / subpage to Matthew to add | Authority backlinks lift ranking; Maison's real press credibility is a moat mass brands lack |

## Session Log

### 2026-07-04 — Week-2 live call synthesized → folded into context (Maddy)
- Read the full Week-2 call transcript (`call transcript w2 .srt`; auto-captions badly garbled — mis-transcriptions normalized, load-bearing figures/brand names flagged as uncertain). Structure: Matthew presents the technical build + config; Maddy presents insight; Chi / Michelle / Hà = Maison team.
- **State change since 27/6 — website is LIVE** with real (tiny) global traffic. Search Console **88 impressions / 33 clicks**; GA4 ~**49 active users / 374 clicks**; visitors from US(12)/AU(6)/UK/DE/FR/CA. Pages: Home, Collection, Booking, About. Booking form → email (Google Calendar auto-scheduling due this week). Team added to GA4 + GSC. Structured-data backlink declaration to authority press domains done. → **MD-040, MD-045**; **resolves OQ-001** (booking email = dedicated account Matthew created).
- **Strategy (MD-041, MD-042, MD-043):** priority = website + content; two workstreams in parallel; **ads deferred**; site structure built jointly (two-lane RTW vs Bespoke/Couture + trust + SEO product pages); **Pinterest adopted**; keep FB/IG for branding.
- **New constraint (MD-044) → OQ-015:** Maison's FB + IG Business Manager is **ad-restricted**; paid social blocked; Maddy to seek a fix by ~Thu, fallback new IG. Confirms earlier adjacency finding: luxury-watch/jewelry/hotel buyer data **can't be bought**, only tested via Google/Meta config (Meta stronger), measured by branded-search lift.
- **Diaspora shift now central → OQ-014:** Chi says the overseas/Việt Kiều ratio jumped sharply this year; a creative-director contact independently named the diaspora-student segment as the ideal customer. Chi leans **overseas-market-first** (diaspora share a "common language" with the brand, use websites more than domestic buyers, "Google is a wide-open door") — in tension with Michelle's logistics concern + domestic-first phasing → **unresolved**.
- **Positioning boundary sharpened:** Chi drew a line that ultra-couture houses (Sabyasachi & the "mấy tỷ" gowns) are **above** Maison's tier → Maison = premium made-to-measure / couture-**adjacent**, NOT ultra-couture. → `founder-signals.md`, `positioning.md`.
- **Insight confirmed:** couture/designer demand is name-led (75–100% branded) → prioritize brand recall, not chasing every keyword; only Công Trí & Phan Huy are SEO-scrapeable among VN designers; optimize subpages (collection/wedding), not just home; wedding search peaks Mar–Apr.
- **Brand-name QA recurred** (Google showing stale wrong logo/name) — Matthew had already fixed it on-site the prior week; it's a **Google re-index lag**, expected to update within the day. → `founder-signals.md §6`.
- **Tone:** warmer / "on track hơn lần trước" (Chi); Chi still drills specifics; **Michelle's audio was broken (chat-only)** → follow up her strategic input in writing; Matthew candid about the unresolved Meta blocker.
- Files touched: `decisions.md` (MD-040–045 + this log), `open-questions.md` (OQ-001 resolved; OQ-014, OQ-015 added; OQ-013 note), `timeline.md`, `client-context/founder-signals.md`, `client-context/personas.md`, `markets/_pointer.md`, `meta/changelog.md`. Source: `call transcript w2 .srt`.
